...
class CarGame {
    constructor() {
        ...
        this.laneWidth = 4.5;
        this.playerLane = 2.25; // Left lane
        this.trafficLane = -2.25; // Right lane
    ...
    createIntersection() {
        ...
        island.position.set(0, 0.15, 100);
	...
	this.scene.add(carGroup);
	this.traffic.push({ mesh: carGroup, speed: 1.5 + Math.random() * 0.5, lane: laneX });
    }
    ...
    handleInput() {
        ...
        if (this.keys['arrowdown'] || this.keys['s']) {
            this.speed = Math.max(this.speed - this.deceleration * 2, -this.maxSpeed * 0.5);
        }
        ...
        // Constrain to road
	this.playerCar.position.x = Math.max(-7.8, Math.min(7.8, this.playerCar.position.x)); 
        // this.playerCar.position.x = Math.max(-6, Math.min(6, this.playerCar.position.x));
    }
	...
	this.traffic.forEach(car => {
	    ...
	    // Respawn traffic car in same lane
	    if (car.mesh.position.z < -30) {
		car.mesh.position.z = 250;
		car.mesh.position.x = car.lane;  // Keep in same lane
	    }
